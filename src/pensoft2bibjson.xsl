...
<xsl:variable name="source" select="//article-meta/article-id[@pub-id-type='doi']"></xsl:variable>	
...
<xsl:template name="escapeQuote">
	...
	<xsl:template match="ref">
		...
		<xsl:text>{&#x0D;</xsl:text>
		<!-- <xsl:text>"_id":"</xsl:text><xsl:value-of select="@id"/><xsl:text>"</xsl:text> -->
		<xsl:text>"_id":"</xsl:text><xsl:value-of select="$source"/><xsl:text>#</xsl:text><xsl:value-of select="@id"/><xsl:text>"</xsl:text>
				...
				<xsl:for-each select="ext-link">
					<xsl:choose>
						...
						<xsl:when test="@ext-link-type='doi'">
							<xsl:text>{"type":"doi",</xsl:text>
							<xsl:text>"id":"</xsl:text><xsl:value-of select="@xlink:href"/><xsl:text>"</xsl:text>
							<xsl:text>}</xsl:text>
						</xsl:when>
						<xsl:otherwise>
						</xsl:otherwise>
						...
						<xsl:when test="@ext-link-type='uri'">
							...
							<xsl:choose>
						    	<xsl:when test="contains(@xlink:href, '.pdf')">
									<xsl:text>"anchor":"PDF",</xsl:text>
								</xsl:when>
								<xsl:otherwise>
									<xsl:text>"anchor":"LINK",</xsl:text>
								</xsl:otherwise>
							</xsl:choose>	
							<xsl:text>"url":"</xsl:text><xsl:value-of select="@xlink:href"/><xsl:text>"</xsl:text>
							<xsl:text>}</xsl:text>
						</xsl:when>
						<xsl:otherwise>
						</xsl:otherwise>
